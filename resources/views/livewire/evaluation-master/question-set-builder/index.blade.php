{{-- question-set-builder/index.blade.php — Step 2 + 3 builder --}}
<div>
    {{-- ══ Page Header ══════════════════════════════════════ --}}
            ...
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 small">
                    <li class="breadcrumb-item">
                        <a href="{{ route('admin.question-sets.index') }}" wire:navigate>Question Sets</a>
                    </li>
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-9">
            ...
            <div class="d-flex gap-2 mb-4">
                ...
                <span class="badge bg-primary px-3 py-2"><i class="ri ri-question-line me-1"></i>Step 3 — Questions</span>
            </div>

            {{-- ── Group Form (inline) ─────────────────────── --}}
            @if ($showGroupForm)
                @include('livewire.evaluation-master.question-set-builder.partials.group-form')
            @endif
            ...
            @if (count($groups) === 0 && ! $showGroupForm)
                <div class="card border-dashed border-2 shadow-none text-center py-5 mb-4">
                    <div class="card-body">
                        ...
                        <button type="button" wire:click="openNewGroup"
            ...
            @else
                @foreach ($groups as $groupIdx => $group)
                    @include('livewire.evaluation-master.question-set-builder.partials.group-card', [
                        'group'    => $group,
                        'groupIdx' => $groupIdx,
                    ])
                @endforeach
        ...
        <div class="col-lg-3">
            @include('livewire.evaluation-master.question-set-builder.partials.set-summary')
        </div>
    ...
    @if ($showPicker)
        @include('livewire.evaluation-master.question-set-builder.partials.question-picker')
    @endif
...
<style>
    ...
    .group-card:hover { box-shadow: 0 4px 12px rgba(0,0,0,.08) !important; }
    .question-row { transition: background .15s; }
    .question-row:hover { background: var(--bs-light); }
