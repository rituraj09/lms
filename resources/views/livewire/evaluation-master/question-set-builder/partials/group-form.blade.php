    ...
    <div class="card-body p-4">
        <div class="row g-3">
            ...
            <div class="col-md-6">
                ...
                @error('groupTitle') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>

            {{-- Category --}}
            <div class="col-md-6">
                <label class="form-label fw-medium small">Question Category <span class="text-danger">*</span></label>
                <select wire:model="groupCategory"
                    class="form-select @error('groupCategory') is-invalid @enderror">
                    <option value="optional">Optional</option>
                    <option value="follow-up question">Follow-up Question</option>
                    <option value="open-text">Open Text</option>
                </select>
                @error('groupCategory') <div class="invalid-feedback">{{ $message }}</div> @enderror
            </div>

            {{-- Instructions --}}
            <div class="col-12">
                <label class="form-label fw-medium small">Instructions <span class="text-muted">(optional)</span></label>
            ...
            <div class="col-12">
                <div class="row g-2">
                    @foreach ([
                        ['model' => 'groupRandomize',     'id' => 'grp-rand',      'label' => 'Randomize Questions',    'hint' => 'Shuffle question order in this group'],
                        ['model' => 'groupAllowMainBack', 'id' => 'grp-mainback',  'label' => 'Allow Main Backtrack',   'hint' => 'Candidate can revisit the main stimulus (Memory Recall = false)'],
                        ['model' => 'groupAllowBack',     'id' => 'grp-back',      'label' => 'Allow Backtrack',        'hint' => 'Candidate can go back within group (Rapid Fire = false)'],
                        ['model' => 'groupMainTimer',     'id' => 'grp-maintimer', 'label' => 'Main Timer',             'hint' => 'Group uses a single countdown timer (Memory Recall = true)'],
                    ] as $toggle)
                        <div class="col-md-6">
