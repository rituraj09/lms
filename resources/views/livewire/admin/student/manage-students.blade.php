...
<div class="min-h-screen bg-light py-4 py-lg-5">
    <div class="container-xl">
        ...
        @if ($maxStudentCount > 0)
            <div
                class="alert {{ $canEnrollStudent ? 'alert-info' : 'alert-danger' }} d-flex align-items-center justify-content-between mb-4">
                        ...
                        @forelse($students as $student)
                            ...
                            <tr wire:key="student-{{ $student->id }}" class="student-row">
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    <div class="d-flex flex-wrap gap-1 mt-2">
                                        @foreach($assessmentConfig as $type => [$badgeColor, $typeLabel, $typeIcon])
                                            ...
                                            <span class="badge bg-{{ $badgeColor }} bg-opacity-15 text-white d-inline-flex align-items-center gap-1 px-2 py-1"
                                                  style="font-size: 0.7rem; font-weight: 500;"
                                                  title="{{ $promotionName ? $typeLabel . ': ' . $promotionName : $typeLabel . ': Not assigned' }}">
                                                    <i class="ri {{ $typeIcon }}" style="font-size: 0.75rem;"></i>
                                                    <span>{{ $typeLabel }}</span>
                                                    @if($promotionName)
                                                        <span class="ms-1">·</span>
                                                        <span>{{ Str::limit($promotionName, 12) }}</span>
                                                    @else
                                ...
                                <td class="px-4 py-3">
                                    ...
                                    @endphp
                                    <span
                                        class="badge bg-{{ $color }} bg-opacity-10 text-{{ $color }} d-inline-flex align-items-center gap-1 px-3 py-2">
                                            <i class="ri {{ $icon }}"></i>
                                            {{ $label }}
                                        </span>
                                </td>
    ...
    <div class="toast-container position-fixed bottom-0 end-0 p-3" style="z-index: 9999;">
        <div id="notifyToast" x-data="{
            ...
            init() {
             ...
             :class="type === 'success' ? 'bg-success' : 'bg-danger'" role="alert" style="display: none;">
